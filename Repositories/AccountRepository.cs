...
namespace GestioneAccounts.DataAccess.Repositories
{
  public class AccountRepository(ApplicationDbContext applicationDbContext) :  IAccountRepository
  {
    ...
    public Task<ICollection<Account>> SearchAccounts(string? nome, DateTime? dataCreazione, string? cognome, string? voce)
    {
      ...
      if (!string.IsNullOrEmpty(cognome))
        query = query.Where(a => a.Cognome.Contains(cognome));
